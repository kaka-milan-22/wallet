...
|---|---|---|
| **Hardware wallet integration (Ledger / Trezor)** | Hot mnemonic in alice (AnB) is fine for testnet and small daily-spend hot accounts. Anything beyond a few hundred dollars belongs on a device that never exposes the seed. **Interim hardening (current, AnB v2):** the master key never lives on the client — it stays in `bob` (Argon2id-wrapped, `mlock`'d, zeroized on idle TTL), so the old "read keychain-derived key + ciphertext → decrypt offline" path is gone. Tighten further with a short `bob serve --ttl`, a **dedicated** alice identity for the wallet (this bob is shared with other consumers, so a single allow-all cert gives no isolation) scoped in `authz.json` to the `wallet-` key prefix, and keeping `alice`'s `client.key` 0600. **Note:** AnB v2 has **no per-key Touch ID gate** (agent-vault 0.5.0's `--require-presence`, recommended in `1.09`, is no longer available — superseded by off-disk custody). A same-uid attacker with `client.key` can still drive a running, unlocked bob, and the binary-replacement / post-decrypt memory-dump gaps remain — none of this raises the resilience threshold above ~$1.5k. Hardware-backed `alice` client key (Secure Enclave / PKCS#11) is the AnB roadmap item that re-adds a presence gate. **Full rationale + threat model + integration sketch:** [`docs/why_hard_wallet.md`](docs/why_hard_wallet.md). | Refactor `core/signer.py` to dispatch on `account.account_type` (`hd_mnemonic` vs `ledger`). Wallet builds the unsigned tx; signing happens on-device with user button press. Keystore-backed accounts and Ledger accounts coexist in `state.json`. |
...
| **alice binary integrity** | `shutil.which("alice")` follows PATH. Any `alice` shim earlier in PATH could exfiltrate every secret it can resolve on first call (it holds the client cert that authenticates to bob). | Pin absolute path in `~/.wallet/config.json` plus sha256 verification on every wallet startup. Fail loudly on mismatch. (Bob-side, `authz.json` prefix scoping limits the blast radius of a stolen client identity.) |
...
| **Audit log encryption-at-rest** | Plain JSON-lines audit log reveals address activity to anyone with file-system access. | Optional age encryption with a key stored in alice (AnB). `wallet audit decrypt` (TTY-only) for forensic review. |
